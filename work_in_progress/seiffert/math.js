...
bernoulli2nN = [
  ...
  2929993913841559n,
  -261082718496449122051n,
  1520097643918070802691n,
  -27833269579301024235023n,
  596451111593912163277961n,
  -5609403368997817686249127547n,
  495057205241079648212477525n,
  -801165718135489957347924991853n,
  29149963634884862421418123812691n,
  -2479392929313226753685415739663229n,
  84483613348880041862046775994036021n,
  -1215233140483755572040304994079820246041491n,
  12300585434086858541953039857403386151n,
  -106783830147866529886385444979142647942017n,
  1472600022126335654051619428551932342241899101n,
  -78773130858718728141909149208474606244347001n,
  1505381347333367003803076567377857208511438160235n,
  -5827954961669944110438277244641067365282488301844260429n,
  34152417289221168014330073731472635186688307783087n,
  -24655088825935372707687196040585199904365267828865801n,
  414846365575400828295179035549542073492199375372400483487n,
  ...
  -2024576195935290360231131160111731009989917391198090877281083932477n,
  660714619417678653573847847426261496277830686653388931761996983n,
  ...
  1220813806579744469607301679413201203958508415202696621436215105284649447n,
  -211600449597266513097597728109824233673043954389060234150638733420050668349987259n,
...
function resetPrecisionScale() {
  setPrecisionScale("reset");
}
...
function isArbitrary(x) {
  return typeof x === "bigint" || typeof x.re === "bigint";
}
  ...
  if (isComplex(x)) {
    if (isArbitrary(x)) {
      ...
      if (abs(x.re) < abs(x.im))
        return mul(
          abs(x.im),
          sqrt(arb1 + div(div(mul(x.re, x.re), x.im), x.im))
        );
      else
        return mul(
          abs(x.re),
          sqrt(arb1 + div(div(mul(x.im, x.im), x.re), x.re))
        );
    }
    ...
    else return Math.abs(x.re) * Math.sqrt(1 + (x.im / x.re) ** 2);
  }
...
function madd(x, y) {
  if (arguments.length > 2) {
    var z = madd(x, y);
    for (var i = 2; i < arguments.length; i++) z = madd(z, arguments[i]);
    return z;
  }
...
function mul(x, y) {
  ...
  if (isComplex(x) || isComplex(y)) {
    ...
    if (isArbitrary(x))
      return {
        re: (x.re * y.re - x.im * y.im) / precisionScale,
        im: (x.im * y.re + x.re * y.im) / precisionScale,
      };

    return { re: x.re * y.re - x.im * y.im, im: x.im * y.re + x.re * y.im };
  }
...
function mdiv(x, y) {
  if (isComplex(x) || isComplex(y)) {
    ...
    if (isArbitrary(x)) {
      var N = { re: x.re * y.re + x.im * y.im, im: x.im * y.re - x.re * y.im };
      var D = y.re * y.re + y.im * y.im;

      return {
        re: (precisionScale * N.re) / D,
...
function minv(x) {
  if (isArbitrary(x)) return mdiv(arb1, x);
...
function mpow(x, y) {
  ...
  if (isComplex(x) || isComplex(y)) {
    ...
    if (x.re === 0 && x.im === 0 && y.re > 0) return complex(0);
    if (x.re === 0 && x.im === 0 && y.re === 0 && y.im === 0) return complex(1);
    if (x.re === 0 && x.im === 0 && y.re < 0) throw Error("Power singularity");

    return mexp(mul(y, mmlog(x)));
  }

  if (x === 0 && y < 0) throw Error("Power singularity");

  if (x < 0 && !Number.isInteger(y)) return mpow(complex(x), y);

  return x ** y;
}

function root(x, y) {
  return mpow(x, mdiv(1, y));
}
...
function msqrt(x) {
  if (isComplex(x)) {
    if (isArbitrary(x)) {
      if (x.im === 0n)
        if (x.re < 0n) return { re: 0n, im: msqrt(-x.re) };
        else return { re: msqrt(x.re), im: 0n };
      ...
      return {
        re: sqrt(div(c + x.re, arb2)),
        im: sign * sqrt(div(c - x.re, arb2)),
      };
    }
...
function besselJ(n, x) {
  ...
  return (
    ((x / 2) ** n * hypergeometric0F1(n + 1, -0.25 * x ** 2)) / gamma(n + 1)
  );
}
...
function besselJZero(n, m, derivative = false) {
  ...
  if (derivative) {
    ...
    if (n < 0)
      throw Error("Negative order for Bessel derivative zero not supported");

    return findRoot(
      (x) => diff((x) => besselJ(n, x), x),
...
function besselY(n, x) {
  if (isComplex(n) || isComplex(x)) {
    if (isZero(x)) {
      ...
      if (fractionalPart(n.re) === -0.5 && n.im === 0) return complex(0);
    }
    ...
    if (isInteger(n))
      return div(
        add(
          diff((n) => besselJ(n, x), n),
          mul(
            pow(-1, n),
            diff((n) => besselJ(n, x), neg(n))
          )
        ),
        pi
      );
  ...
  if (x === 0) if (fractionalPart(n) === -0.5) return 0;

  // dlmf.nist.gov/10.2.4
  if (Number.isInteger(n))
    return (
      (diff((n) => besselJ(n, x), n) +
        (-1) ** n * diff((n) => besselJ(n, x), -n)) /
      pi
    );
...
function besselYZero(n, m, derivative = false) {
  ...
  if (derivative) {
    ...
    if (n < 0)
      if (fractionalPart(n) > -0.5) e += fractionalPart(n) * pi;
      else e += (1 + fractionalPart(n)) * pi;

    return findRoot(
      (x) => besselY(n, x),
      [m === 1 && fractionalPart(n) > -0.5 ? 1e-10 : e - delta, e + delta]
    );
  }
}
...
function besselK(n, x) {
  ...
  if (isComplex(n) || isComplex(x)) {
    ...
    if (abs(x) > useAsymptotic) {
      ...
      var t2 = hypergeometric2F0(add(n, 0.5), sub(0.5, n), div(-0.5, x));
    ...
    if (isInteger(n))
      return mul(
        ...
        0.5,
        add(
          diff((n) => besselI(n, x), n),
          diff((n) => besselI(n, x), neg(n))
        )
      );
  ...
  if (x > useAsymptotic)
    return (
      sqrt(pi / 2 / x) * exp(-x) * hypergeometric2F0(n + 0.5, 0.5 - n, -0.5 / x)
    );
  ...
  if (Number.isInteger(n))
    return (
      ((-1) ** (n + 1) / 2) *
      (diff((n) => besselI(n, x), n) + diff((n) => besselI(n, x), -n))
    );
...
function hankel1(n, x, tolerance = 1e-10) {
  ...
  // dlmf.nist.gov/10.17
  if (x.im > 0 && mabs(x) > useAsymptotic) {
    var w = madd(x, mul(n, -pi / 2), -pi / 4);
  ...
  if (isInteger(n))
    return madd(besselJ(n, x), mul(complex(0, 1), besselY(n, x)));

  // one less call to besselJ
  var s = msub(
    mul(besselJ(n, x), mexp(mul(n, complex(0, -pi)))),
    besselJ(neg(n), x)
  );
...
function hankel2(n, x, tolerance = 1e-10) {
  ...
  if (x.im < 0 && mabs(x) > useAsymptotic) {
    ...
    return mul(mdiv(Math.sqrt(2 / pi), msqrt(x)), mexp(mul(minusI, w)), s);
  }

  // not duplicate special case
  if (isInteger(n))
    return msub(besselJ(n, x), mul(complex(0, 1), besselY(n, x)));

  // one less call to besselJ
  var s = msub(
    mul(besselJ(n, x), exp(mul(n, complex(0, pi)))),
    besselJ(neg(n), x)
  );
  return mdiv(mul(complex(0, -1), s), msin(mul(n, pi)));
}
  ...
  if (isComplex(x)) {
    ...
    if (x.re < 0) {
      ...
      return mul(
        1 / 3,
        sqrt(neg(x)),
        add(besselJ(1 / 3, z), besselJ(-1 / 3, z))
      );
    }
...
function airyBi(x) {
  if (isComplex(x)) {
    ...
    if (x.re < 0) {
      ...
      return mul(
        sqrt(div(neg(x), 3)),
        sub(besselJ(-1 / 3, z), besselJ(1 / 3, z))
      );
    }
  ...
  if (isComplex(x)) {
    ...
    if (x.re < 0) {
      ...
      return mul(
        1 / sqrt(3),
        neg(x),
        add(besselJ(2 / 3, z), besselJ(-2 / 3, z))
      );
    }
...
function sphericalBesselJ(n, x) {
  return mul(div(Math.sqrt(pi / 2), sqrt(x)), besselJ(add(n, 0.5), x));
}

function sphericalBesselY(n, x) {
  return mul(div(Math.sqrt(pi / 2), sqrt(x)), besselY(add(n, 0.5), x));
}

function sphericalHankel1(n, x) {
  return mul(div(Math.sqrt(pi / 2), sqrt(x)), hankel1(add(n, 0.5), x));
}

function sphericalHankel2(n, x) {
  return mul(div(Math.sqrt(pi / 2), sqrt(x)), hankel2(add(n, 0.5), x));
}

function struveH(n, x) {
  return mul(
    pow(x, add(n, 1)),
    inv(mul(pow(2, n), sqrt(pi), gamma(add(n, 3 / 2)))),
...
function jacobiTheta(n, x, q, tolerance = 1e-10) {
  if (mabs(q) >= 1) throw Error("Unsupported elliptic nome");
  ...
  if (isComplex(x) || isComplex(q)) {
    ...
    var piTau = mdiv(mlog(q), complex(0, 1));
    ...
    switch (n) {
      case 1:
        ...
        while (Math.abs(p.re) > tolerance || Math.abs(p.im) > tolerance) {
          p = mul((-1) ** i, mpow(q, i * i + i), msin(mul(2 * i + 1, x)));
          s = madd(s, p);
          i++;
        }

        return mul(2, mpow(q, 1 / 4), s);
      ...
      case 2:
        ...
        while (Math.abs(p.re) > tolerance || Math.abs(p.im) > tolerance) {
          p = mul(mpow(q, i * i + i), mcos(mul(2 * i + 1, x)));
          s = madd(s, p);
          i++;
        }
      ...
      case 3:
        ...
        while (Math.abs(p.re) > tolerance || Math.abs(p.im) > tolerance) {
          p = mul(mpow(q, i * i), mcos(mul(2 * i, x)));
          s = madd(s, p);
          i++;
        }

        return madd(1, mul(2, s));
      ...
      case 4:
        ...
        while (Math.abs(p.re) > tolerance || Math.abs(p.im) > tolerance) {
          p = mul(mpow(neg(q), i * i), mcos(mul(2 * i, x)));
          s = madd(s, p);
          i++;
        }
...
function ellipticNome(m) {
  ...
  if (isComplex(m))
    return mexp(mdiv(mul(-pi, ellipticK(msub(1, m))), ellipticK(m)));

  if (m > 1) return ellipticNome(complex(m));

  if (m < 0)
    return -mexp((-PI * ellipticK(1 / (1 - m))) / ellipticK(m / (m - 1)));

  return mexp((-PI * ellipticK(1 - m)) / ellipticK(m));
}
...
function fundamentalParallelogram(x, p1, p2) {
  ...
  return madd(x, mul(-Math.floor(m), p1), mul(-Math.floor(n), p2));
}
...
function sn(x, m) {
  if (m > 1 || isComplex(x) || isComplex(m)) {
    ...
    // dlmf.nist.gov/22.17
    if (mabs(m) > 1) return mul(minv(msqrt(m)), sn(mul(msqrt(m), x), minv(m)));

    // periods 4K, 2iK'
    var p1 = mul(4, ellipticK(m));
    var p2 = mul(complex(0, 2), ellipticK(msub(1, m)));

    x = fundamentalParallelogram(x, p1, p2);

    var q = ellipticNome(m);
    var t = mdiv(x, mpow(jacobiTheta(3, 0, q), 2));

    return mul(
      mdiv(jacobiTheta(3, 0, q), jacobiTheta(2, 0, q)),
      mdiv(jacobiTheta(1, t, q), jacobiTheta(4, t, q))
    );
  }

  // dlmf.nist.gov/22.5.ii
  if (m === 0) return msin(x);
  ...
  if (m < 0)
    return (
      ...
      mdiv(jacobiTheta(1, t, q), jacobiTheta(2, 0, q)).re
    );
...
function cn(x, m) {
  if (m > 1 || isComplex(x) || isComplex(m)) {
    ...
    // dlmf.nist.gov/22.17
    if (mabs(m) > 1) return dn(mul(msqrt(m), x), minv(m));

    // periods 4K, 2K + 2iK'
    var p1 = mul(4, ellipticK(m));
    var p2 = madd(mdiv(p1, 2), mul(complex(0, 2), ellipticK(msub(1, m))));
    ...
    return mul(
      mdiv(jacobiTheta(4, 0, q), jacobiTheta(2, 0, q)),
      mdiv(jacobiTheta(2, t, q), jacobiTheta(4, t, q))
    );
  }

  // dlmf.nist.gov/22.5.ii
  if (m === 0) return mcos(x);
  ...
  if (m < 0)
    return (
      (jacobiTheta(4, 0, q) / jacobiTheta(4, t, q)) *
      mdiv(jacobiTheta(2, t, q), jacobiTheta(2, 0, q)).re
    );

  return (
    ((jacobiTheta(4, 0, q) / jacobiTheta(2, 0, q)) * jacobiTheta(2, t, q)) /
    jacobiTheta(4, t, q)
  );
}
...
function dn(x, m) {
  if (m > 1 || isComplex(x) || isComplex(m)) {
    ...
    // dlmf.nist.gov/22.17
    if (mabs(m) > 1) return cn(mul(sqrt(m), x), minv(m));

    // periods 2K, 4iK'
    var p1 = mul(2, ellipticK(m));
    var p2 = mul(complex(0, 4), ellipticK(msub(1, m)));
    ...
    return mul(
      mdiv(jacobiTheta(4, 0, q), jacobiTheta(3, 0, q)),
      mdiv(jacobiTheta(3, t, q), jacobiTheta(4, t, q))
    );
  }
  ...
  return (
    ((jacobiTheta(4, 0, q) / jacobiTheta(3, 0, q)) * jacobiTheta(3, t, q)) /
    jacobiTheta(4, t, q)
  );
}
  ...
  if (m > 1 || isComplex(x) || isComplex(m)) {
    ...
    if (m.im === 0 && m.re <= 1) {
      ...
      if (m.re < 0) {
        ...
        if (p & 1) return msub(n * pi, arcsin(sn(x, m)));
      }

      return madd(arcsin(sn(x, m)), n * pi);
    }
...
function weierstrassRoots(g2, g3) {
  function cubicTrigSolution(p, q, n) {
    ...
    return mul(
      ...
      cos(
        sub(
          div(arccos(mul((3 * sqrt(3)) / 2, q, pow(p, -3 / 2))), 3),
          (2 * pi * n) / 3
        )
      )
    );
...
function weierstrassInvariants(w1, w3) {
  ...
  var g3 = mul(
    ...
    add(
      pow(a, 12),
      mul(-3 / 2, pow(a, 8), pow(b, 4)),
      mul(-3 / 2, pow(a, 4), pow(b, 8)),
      pow(b, 12)
    )
  );
...
function carlsonRC(x, y) {
  if (x < 0 || y < 0 || isComplex(x) || isComplex(y)) {
    ...
    return div(
      arccos(div(sqrt(x), sqrt(y))),
      mul(sqrt(y), sqrt(sub(1, div(x, y))))
    );
  }
...
function carlsonRF(x, y, z, tolerance = 1e-10) {
  if (isComplex(x) || isComplex(y) || isComplex(z)) {
    ...
    var Am = (A0 = mdiv(madd(x, y, z), 3));
    var Q =
      Math.pow(3 * tolerance, -1 / 6) *
      Math.max(mabs(msub(A0, x)), mabs(msub(A0, y)), mabs(msub(A0, z)));
    var g = 0.25;
    var pow4 = 1;
    ...
    while (true) {
      ...
      var ys = sqrt(ym);
      var zs = sqrt(zm);
      var lm = madd(mul(xs, ys), mul(xs, zs), mul(ys, zs));
      var Am1 = mul(madd(Am, lm), g);
      xm = mul(madd(xm, lm), g);
      ym = mul(madd(ym, lm), g);
      zm = mul(madd(zm, lm), g);
    ...
    return (
      (Math.pow(Am, -0.5) *
        (9240 - 924 * E2 + 385 * E2 ** 2 + 660 * E3 - 630 * E2 * E3)) /
      9240
    );
  }
}
...
function ellipticF(x, m) {
  ...
  if (isComplex(x) || isComplex(m)) {
    ...
    return madd(
      mul(
        msin(x),
        carlsonRF(mul(mcos(x), mcos(x)), msub(1, mul(m, msin(x), msin(x))), 1)
      ),
      period
    );
  } else {
...
function ellipticE(x, m) {
  ...
  if (isComplex(x) || isComplex(m)) {
    ...
    return add(
      mul(
        sin(x),
        carlsonRF(mul(cos(x), cos(x)), sub(1, mul(m, sin(x), sin(x))), 1)
      ),
      mul(
        -1 / 3,
        m,
        pow(sin(x), 3),
        carlsonRD(mul(cos(x), cos(x)), sub(1, mul(m, sin(x), sin(x))), 1)
      ),
      period
    );
  } else {
    ...
    return (
      sin(x) * carlsonRF(cos(x) ** 2, 1 - m * sin(x) ** 2, 1) -
...
function jacobiZeta(x, m) {
  ...
  return sub(
    ellipticE(x, m),
    mul(ellipticF(x, m), ellipticE(m), inv(ellipticK(m)))
  );
}
...
function multinomial(n) {
  ...
  for (var i = 0; i < arguments.length; i++) {
    ...
    // standard factorial
    else p = mul(p, factorial(arguments[i]));
  }
...
function logGamma(x) {
  if (isArbitrary(x)) {
    ...
    if (isNegativeIntegerOrZero(arbitrary(x)))
    ...
    while (compare()) {
      ...
      p = div(div(bernoulli2nN[k], bernoulli2nD[k]), mul(K + arb1, K, y));
      s = add(s, p);
  ...
  if (isComplex(x)) {
    ...
    if (x.re < 0) {
      ...
      if (isNegativeIntegerOrZero((x.re + 0.5) / 2))
...
function gamma(x, y, z) {
  if (arguments.length === 2) {
    if (isZero(x)) {
      ...
      if (y < 0 || (y.re < 0 && y.im === 0))
        result = sub(result, complex(0, pi));
...
function beta(x, y, z, w) {
  if (arguments.length === 4) return sub(beta(y, z, w), beta(x, z, w));

  if (arguments.length === 3)
    return mul(
      pow(x, y),
      inv(y),
      hypergeometric2F1(y, sub(1, z), add(y, 1), x)
    );
...
function betaRegularized(x, y, z, w) {
  if (arguments.length === 4) return div(beta(x, y, z, w), beta(z, w));
...
function erf(x) {
  ...
  return mul(2 / sqrt(pi), x, hypergeometric1F1(0.5, 1.5, neg(mul(x, x))));
}
...
function erfc(x) {
  ...
  if (abs(x) > useAsymptotic && (absArg < pi / 4 || absArg > (3 * pi) / 4)) {
    ...
    var t = mul(
      ...
      exp(neg(mul(x, x))),
      inv(x),
      hypergeometric2F0(0.5, 1, neg(inv(mul(x, x))))
    );
...
function fresnelS(x) {
  ...
  var m1 = hypergeometric1F1(0.5, 1.5, mul(complex(0, pi / 2), pow(x, 2)));
  var m2 = hypergeometric1F1(0.5, 1.5, mul(complex(0, -pi / 2), pow(x, 2)));

  var result = mul(x, sub(m1, m2), complex(0, -0.5));
...
function fresnelC(x) {
  ...
  var result = mul(x, add(m1, m2), 0.5);
...
function expIntegralEi(x, adjustImForGamma = false, tolerance = 1e-10) {
  ...
  if (isUnity(neg(x)))
    return complexAverage(
      (x) => expIntegralEi(x, adjustImForGamma, tolerance),
      x
    );
  ...
  if (isComplex(x)) {
    if (abs(x) > useAsymptotic) {
      var s = complex(1);
      var p = complex(1),
        pLast = p; // mul returns new object
      var i = 1;
      ...
      if (adjustImForGamma) return mul(s, exp(x), inv(x));
      else {
...
function sinIntegral(x) {
  ...
  var result = mul(
    complex(0, 0.5),
    add(gamma(0, neg(ix)), neg(gamma(0, ix)), log(neg(ix)), neg(log(ix)))
  );
...
function cosIntegral(x) {
  ...
  var result = sub(
    log(x),
    mul(0.5, add(gamma(0, neg(ix)), gamma(0, ix), log(neg(ix)), log(ix)))
  );
...
function sinhIntegral(x) {
  ...
  var result = mul(
    0.5,
    add(gamma(0, x), neg(gamma(0, neg(x))), log(x), neg(log(neg(x))))
  );
...
function coshIntegral(x) {
  ...
  return mul(
    -0.5,
    add(gamma(0, x), gamma(0, neg(x)), neg(log(x)), log(neg(x)))
  );
}
...
function hypergeometric0F1(a, x, tolerance = 1e-10) {
  ...
  if (isComplex(a) || isComplex(x)) {
    // asymptotic form as per Johansson arxiv.org/abs/1606.06977
    if (abs(x) > useAsymptotic) {
      if (isNegativeIntegerOrZero(sub(a, 0.5))) {
...
function hypergeometric1F1(a, b, x, tolerance = 1e-10) {
  ...
  if (isArbitrary(x)) {
    ...
    if (isNegativeIntegerOrZero(arbitrary(b)))
    ...
    if (isComplex(a) || isComplex(b) || isComplex(x)) {
      ...
      if (x.re < 0n)
        return mul(exp(x), hypergeometric1F1(sub(b, a), b, neg(x)));
  ...
  if (isComplex(a) || isComplex(b) || isComplex(x)) {
    ...
    // asymptotic form is complex
    if (Math.abs(x) > useAsymptotic)
      return hypergeometric1F1(a, b, complex(x)).re;
...
function hypergeometricU(a, b, x) {
  ...
  if (abs(x) > useAsymptotic) {
    return mul(
      pow(x, neg(a)),
      hypergeometric2F0(a, add(a, neg(b), 1), neg(inv(x)))
    );
  }
  ...
  if (isPositiveInteger(a) && isEqualTo(a, b))
    return mul(exp(x), gamma(sub(1, a), x));
  ...
  var t2 = mul(
    gamma(sub(1, b)),
    inv(gamma(add(a, neg(b), 1))),
    hypergeometric1F1(a, b, x)
  );
...
function whittakerM(k, m, x) {
  return mul(
    exp(mul(-0.5, x)),
    pow(x, add(m, 0.5)),
    hypergeometric1F1(add(m, neg(k), 0.5), add(mul(2, m), 1), x)
  );
}
...
function whittakerW(k, m, x) {
  return mul(
    exp(mul(-0.5, x)),
    pow(x, add(m, 0.5)),
    hypergeometricU(add(m, neg(k), 0.5), add(mul(2, m), 1), x)
  );
}
  ...
  if (isComplex(a) || isComplex(b) || isComplex(c) || isComplex(x)) {
    ...
    switch (index) {
      ...
      case 1:
        return mul(
          pow(sub(1, x), neg(a)),
          hypergeometric2F1(a, sub(c, b), c, div(x, sub(x, 1)))
        );
...
function hypergeometric1F2(a, b, c, x, tolerance = 1e-10) {
  ...
  if (isComplex(a) || isComplex(b) || isComplex(c) || isComplex(x)) {
    ...
    if (abs(x) > useAsymptotic) {
      ...
      ck[1] = add(
        mul(add(mul(3, a), b, c, -2), aNegbNegc, 1 / 2),
        mul(2, b, c),
        -3 / 8
      );
      ...
      var toSquare = add(
        mul(b, c),
        mul(aNegbNegc, add(mul(3, a), b, c, -2), 1 / 4),
        -3 / 16
      );

      ck[2] = add(
        mul(2, toSquare, toSquare),
      ...
      function w(k) {
        return mul(1 / 2 ** k, ck[k], pow(neg(x), -k / 2));
      }
      ...
      while (abs(wk) > tolerance) {
        ...
        var t1 = mul(
          add(
            3 * k ** 2,
            mul(2 * k, add(mul(-3, a), b, c, -2)),
            mul(3, mul(a, a)),
            neg(mul(sub(b, c), sub(b, c))),
            neg(mul(2, a, add(b, c, -2))),
            1 / 4
          ),
          ck[k - 1]
        );
        ...
        var t2 = mul(
          add(k, neg(a), b, neg(c), -1 / 2),
          add(k, neg(a), neg(b), c, -1 / 2),
          add(k, neg(a), b, c, -5 / 2),
          ck[k - 2]
        );
    ...
    // asymptotic form is complex
    if (Math.abs(x) > useAsymptotic)
      return hypergeometric1F2(a, b, c, complex(x)).re;
...
function hypergeometricSeries(
  A,
  B,
  x,
  complexArguments = false,
  tolerance = 1e-10
) {
  if (complexArguments) {
    var s = complex(1);
...
function hypergeometricPFQ(A, B, x) {
  ...
  if (abs(x) > 1)
    throw Error("General hypergeometric argument currently restricted");
...
function mexp(x) {
  if (isArbitrary(x)) {
    if (isComplex(x)) {
      ...
      return { re: mul(expXre, cos(x.im)), im: mul(expXre, sin(x.im)) };
    }
...
function lambertW(k, x, tolerance = 1e-10) {
  ...
  if (!isComplex(x)) {
    if (k === 0 && x > -expMinusOne)
      return findRoot((w) => w * Math.exp(w) - x, [-1, 1000], {
        tolerance: tolerance,
      });
    if (k === -1 && x > -expMinusOne && x < 0)
      return findRoot((w) => w * Math.exp(w) - x, [-1000, -1], {
        tolerance: tolerance,
      });
    x = complex(x);
  }
  ...
  if (k === 0 && abs(x) < 1.25) {
    ...
    var w = x.re < 0.5 ? complex(0, 0.5 * Math.sign(x.im)) : complex(0);
  } else {
...
function fractionalPart(x) {
  return sub(x, integerPart(x));
}

function mrandom(x, y) {
...
function hermite(n, x) {
  ...
  if (isComplex(n) || isComplex(x)) {
    ...
    var s = sub(
      div(hypergeometric1F1(a, 0.5, pow(x, 2)), gamma(b)),
  ...
  var s =
    hypergeometric1F1(-n / 2, 0.5, x ** 2) / gamma((1 - n) / 2) -
...
function legendreP(l, m, x, renormalized = false) {
  ...
  if (Number.isInteger(l) && Number.isInteger(m) && Math.abs(x) <= 1) {
    ...
    var legendre1 =
      (-1) ** mm *
      Math.sqrt((2 * mm + 1) / 4 / pi / factorial(2 * mm)) *
      factorial2(2 * mm - 1) *
      (1 - x * x) ** (mm / 2);
...
function doubleLambert(n, x, y, tolerance = 1e-10) {
  ...
  function test(n, x, y) {
    return div(
      asymptotic(n, x, y),
      add(1, mul(2 * (-1) ** n, sqrt(x), sqrt(neg(y))))
    );
  }

  function start(n, x, y, tolerance = 1e-3) {
    var a = asymptotic(n, x, y),
      w1,
      w2;
    ...
    if (Math.abs(w1.im - a.im) < Math.abs(w2.im - a.im)) return w1;
    else return w2;
  }
  ...
  throw Error(
    "No double Lambert root found for x = " +
      JSON.stringify(x) +
      " and y = " +
      JSON.stringify(y)
  );
...
function msin(x) {
  if (isArbitrary(x)) {
    if (isComplex(x))
      return {
        re: mul(msin(x.re), cosh(x.im)),
        im: mul(mcos(x.re), sinh(x.im)),
      };

    x = x % TWO_PI;

    // reduce to [-pi/2,pi/2] with successive reductions
    if (x > PI / 2) return msin(PI - x);
    if (x < -PI / 2) return msin(-PI - x);
    ...
    while (p !== 0n) {
      p = mdiv(mul(p, -arb1, x, x), mul(i, i + arb1));
      s += p;
      i += arb2;
    }
  ...
  if (isComplex(x))
    return {
      re: Math.sin(x.re) * Math.cosh(x.im),
...
function mcos(x) {
  ...
  if (isComplex(x))
    return {
      re: Math.cos(x.re) * Math.cosh(x.im),
...
function arctan(x) {
  if (isComplex(x)) {
    var s = sub(
      log(sub(1, mul(complex(0, 1), x))),
      log(add(1, mul(complex(0, 1), x)))
    );
    return mul(complex(0, 0.5), s);
  }
...
function sinh(x) {
  ...
  if (isComplex(x))
    return {
      re: Math.sinh(x.re) * Math.cos(x.im),
...
function cosh(x) {
  ...
  if (isComplex(x))
    return {
      re: Math.cosh(x.re) * Math.cos(x.im),
...
function haversine(x) {
  return div(sub(1, cos(x)), 2);
}

function inverseHaversine(x) {
  return arccos(sub(1, mul(2, x)));
}

// analyticphysics.com / The Complex Gudermannian Function

function gudermannian(x) {
  return mul(2, arctan(tanh(div(x, 2))));
}

function inverseGudermannian(x) {
  return mul(2, arctanh(tan(div(x, 2))));
}
...
function zeta(x, tolerance = 1e-10) {
  ...
  if (x > directSummation || x.re > directSummation) {
    if (isComplex(x)) {
      ...
      while (
        Math.abs(p.re) > tolerance ||
        Math.abs(p.im) > tolerance * tolerance
      ) {
        p = pow(i, neg(x));
        s = add(s, p);
        i++;
  ...
  if (isComplex(x) && x.im !== 0)
    n = Math.max(
      n,
      Math.ceil(log(2 / abs(gamma(x)) / tolerance) / log(3 + sqrt(8)))
    );
...
function hurwitzZeta(x, a, tolerance = 1e-10) {
  ...
  if (isComplex(x) || isComplex(a)) {
    ...
    if (a.re < 0) {
      ...
      return add(
        hurwitzZeta(x, add(a, m)),
        summation((i) => pow(add(a, i), neg(x)), [0, m - 1])
      );
    }
    ...
    if (useArbitrary) {
      ...
      while (p.re !== 0n || p.im !== 0n) {
        ...
        p = div(
          mul(p, add(x, 2n * i - arb2), add(x, 2n * i - arb3)),
      ...
      var result = arbitrary(
        mul(add(S, I, T), pow(add(a, arbN), mul(-arb1, x)))
      );
    ...
    var T = div(add(0.5, t), pow(add(a, n), x));
...
function polylog(n, x, tolerance = 1e-10) {
  ...
  if (abs(x) >= 1) {
    ...
    if (isPositiveInteger(n)) {
      ...
      var t2 = mul(
        div(pow(twoPiI, n), factorial(n)),
        bernoulli(n, div(log(x), twoPiI))
      );

      var y = isComplex(x) ? x : complex(x); // just for test
      var t3 =
        y.im < 0 || (y.im === 0 && y.re >= 1)
          ? mul(twoPiI, div(pow(log(x), n - 1), factorial(n - 1)))
          : 0;
    ...
    var z1 = mul(pow(I, v), hurwitzZeta(v, add(0.5, L)));
    var z2 = mul(pow(I, neg(v)), hurwitzZeta(v, sub(0.5, L)));
...
function ode(f, y, [x0, x1], step = 0.001, method = "runge-kutta") {
  if (x1 < x0) {
    var compare = (x) => x >= x1;
    step *= -1;
  } else var compare = (x) => x <= x1;
  ...
  if (
    ...
    var steps = Math.trunc(absD / abs(step)),
      currentStep = 0;
  }
      ...
      if (isComplex(x0)) {
        ...
        for (var x = add(x0, step); currentStep < steps; x = add(x, step)) {
          ...
          for (var i = 0; i < size; i++)
            y1.push(add(y[i], mul(k1[i], halfStep)));
          var k2 = f(add(x, halfStep), y1);
          for (var i = 0; i < size; i++)
            y2.push(add(y[i], mul(k2[i], halfStep)));
          ...
          for (var i = 0; i < size; i++)
            y[i] = add(
              y[i],
              mul(add(k1[i], mul(2, k2[i]), mul(2, k3[i]), k4[i]), step, 1 / 6)
            );
...
function gradient(f, point) {
  if (f.length !== point.length)
    throw Error("Gradient point length differs from function");
  ...
  switch (method) {
    case "euler-maclaurin":
      ...
      for (var i = 0; i < maxIter; i++) {
        nextEulerIteration();
        result = h * s;
        if (Math.abs(result - previous) < tolerance * Math.abs(previous))
          return result;
        previous = result;
      }
    ...
    case "adaptive-simpson":
      ...
      function adaptiveSimpson(a, b, fa, fm, fb, s, tolerance, depth) {
        ...
        if (isNaN(f1) || isNaN(f2))
          throw Error("NaN encountered in integration");
        ...
        if (Math.abs(error) < tolerance || depth > maxIter) return ss + error;
          ...
          var m = a + h / 2;
          return (
            adaptiveSimpson(a, m, fa, f1, fm, s1, tolerance / 2, depth + 1) +
    ...
    case "gaussian":
      ...
      for (var j = 1; j <= n / 2; j++) {
        var r = Math.cos((Math.PI * (j - 0.25)) / (n + 0.5));
...
function spline(points, value = "function", tolerance = 1e-10) {
  ...
  for (var i = 0; i < A.length; i++) {
    A[i][i] = 2 * (h(i) + h(i + 1));
    y[i] =
      (3 * (a[i + 2] - a[i + 1])) / h(i + 1) - (3 * (a[i + 1] - a[i])) / h(i);
  }
...
function padeApproximant(f, n, d, center = 0) {
  ...
  try {
    ...
    throw Error("Singular Pade matrix encountered");
  }
  b.unshift(1);
...
function fourierSinCoefficient(f, n, period) {
  ...
  if (typeof f === "function") {
    ...
    return (
      (2 / T) *
      integrate((t) => f(t) * sin(((2 * n * pi) / T) * t), [0, T], {
        method: "tanh-sinh",
      })
    );
  }
...
function fourierCosCoefficient(f, n, period) {
  ...
  if (typeof f === "function") {
    ...
    if (n === 0)
      return (1 / T) * integrate((t) => f(t), [0, T], { method: "tanh-sinh" });

    return (
      (2 / T) *
      integrate((t) => f(t) * cos(((2 * n * pi) / T) * t), [0, T], {
        method: "tanh-sinh",
      })
    );
  }
...
function luDecomposition(A, tolerance = 1e-10) {
  ...
  for (var i = 0; i < size; i++) {
    ...
    for (var j = i + 1; j < size; j++) {
      LU[j][i] /= LU[i][i];
      for (var k = i + 1; k < size; k++) LU[j][k] -= LU[j][i] * LU[i][k];
    }
  }

  var L = identity(size);
  for (var i = 1; i < size; i++) for (var j = 0; j < i; j++) L[i][j] = LU[i][j];
...
function luSolve(A, b) {
  ...
  for (var i = 0; i < size; i++)
    for (var j = 0; j < size; j++) pb[i] += lu.P[i][j] * b[j];
...
function mtranspose(A) {
  ...
  for (var i = 0; i < A.length; i++)
    for (var j = 0; j < A[0].length; j++) T[j][i] = A[i][j];
...
function matrixAdd(A, B) {
  ...
  for (var i = 0; i < A.length; i++)
    for (var j = 0; j < A[0].length; j++) C[i][j] = add(A[i][j], B[i][j]);
...
function matrixSub(A, B) {
  if (!Array.isArray(A) && !Array.isArray(B))
    throw Error("No matrices to subtract");
  if (!Array.isArray(A)) A = matrix(B.length, B[0].length, A);
  if (!Array.isArray(B)) B = matrix(A.length, A[0].length, B);
  ...
  for (var i = 0; i < A.length; i++)
    for (var j = 0; j < A[0].length; j++) C[i][j] = sub(A[i][j], B[i][j]);
